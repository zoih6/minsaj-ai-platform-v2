...
(() => {
  ...
  const excluded = (el) => { let n = el; while (n && n !== document.body) { if (n.hasAttribute && n.hasAttribute('data-audit-exclude')) return true; n = n.parentElement; } return false; };
  const all = Array.from(document.querySelectorAll('*')).filter(el => !excluded(el));
  const visible = (el) => {
  ...
  // v4: normalize modern CSS color syntax to legacy rgba for the parser below
  const normColor = (c) => {
    if (!c) return c;
    ...
    if (m) {
      ...
      return `rgba(${Math.round(parseFloat(m[1]) * 255)}, ${Math.round(parseFloat(m[2]) * 255)}, ${Math.round(parseFloat(m[3]) * 255)}, ${a})`;
    }
    return c;
  };
  ...
  const alphaOf = (c) => { c = normColor(c); const m = c.match(/rgba?\([^)]*?,\s*([\d.]+)\)/); return m ? parseFloat(m[1]) : 1; };
  const composite = (top, bottom) => {
    // one-layer alpha composite, returns opaque rgb string
    top = normColor(top); bottom = normColor(bottom); const a = alphaOf(top);
    if (a >= 1) return top;
  ...
  const bgOf = (el) => {
    ...
    while (e && e !== document.documentElement) {
      const bg = normColor(getComputedStyle(e).backgroundColor);
    ...
    if (alphaOf(acc) < 1) { const bodyBg = normColor(getComputedStyle(document.body).backgroundColor); acc = composite(acc, bodyBg && bodyBg.includes('rgba(0, 0, 0, 0)') ? 'rgb(255, 255, 255)' : bodyBg); }
  ...
  for (const el of textEls) {
    ...
    const fg = normColor(s.color), bg = bgOf(el);
    if (fg === bg) continue;
    try {
